...
from unshackle.core.config import Config, _resolve_user_path, get_config_candidates
...
def test_bare_directory_names_resolve_under_home(tmp_path: Path) -> None:
    ...
    assert cfg.directories.downloads == (tmp_path / "downloads").resolve()
    assert cfg.directories.cache == (tmp_path / "cache").resolve()
    assert cfg.directories.watchers == (tmp_path / "watchers").resolve()
...
def test_code_dirs_stay_package_relative() -> None:
    cfg = Config(directories={"vaults": "vaults", "fonts": "fonts"})
    assert cfg.directories.vaults == (cfg.directories.namespace_dir / "vaults").resolve()
    assert cfg.directories.fonts == (cfg.directories.namespace_dir / "fonts").resolve()


def test_resolve_user_path_joins_relative() -> None:
    base = Path("/tmp/unshackle-home")
    assert _resolve_user_path("downloads", relative_to=base) == (base / "downloads").resolve()
