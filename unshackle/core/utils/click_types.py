...
class VideoCodecChoice(click.Choice):
    ...
    def __init__(self, codec_enum: Any) -> None:
...
class LazyVideoCodecList(MultipleVideoCodecChoice):
    """Load video codec enums only when a download command parses -v/--vcodec."""

    name = "multiple_video_codec_choice"

    def __init__(self) -> None:
        self._delegate: Optional[MultipleVideoCodecChoice] = None

    def _loaded(self) -> MultipleVideoCodecChoice:
        if self._delegate is None:
            from unshackle.core.tracks.video import Video

            self._delegate = MultipleVideoCodecChoice(Video.Codec)
        return self._delegate
...
class LazySubtitleCodec(SubtitleCodecChoice):
    """Load subtitle codec enums only when a download command parses --sub-format."""

    def __init__(self) -> None:
        self._delegate: Optional[SubtitleCodecChoice] = None

    def _loaded(self) -> SubtitleCodecChoice:
        if self._delegate is None:
            from unshackle.core.tracks.subtitle import Subtitle

            self._delegate = SubtitleCodecChoice(Subtitle.Codec)
        return self._delegate
...
VIDEO_CODEC_LIST = LazyVideoCodecList()
SUBTITLE_CODEC = LazySubtitleCodec()
